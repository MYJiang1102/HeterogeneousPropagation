...
def train(args, data_info):
    ...
    for step in range(args.n_epoch):
        ...
        while start < train_data.shape[0]:
            ...
            scores,user_dae_loss,item_dae_loss = model(*_get_feed_data(args, train_data, user_triple_set, item_triple_set, start, start + args.batch_size))
            loss = loss_func(scores, labels)+(user_dae_loss+item_dae_loss)*0
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            start += args.batch_size
        eval_auc,eval_acc, eval_f1 = ctr_eval(args, model, eval_data, user_triple_set, item_triple_set)
        test_auc, test_acc,test_f1 = ctr_eval(args, model, test_data, user_triple_set, item_triple_set)
        ctr_info = 'epoch %.2d    eval auc: %.4f acc: %.4f f1: %.4f    test auc: %.4f acc: %.4f f1: %.4f'
        logging.info(ctr_info, step, eval_auc,eval_acc, eval_f1, test_auc,test_acc, test_f1)
        if args.show_topk:
...
def ctr_eval(args, model, data, user_triple_set, item_triple_set):
    ...
    while start < data.shape[0]:
        ...
        scores,user_dae_loss,item_dae_loss = model(*_get_feed_data(args, data, user_triple_set, item_triple_set, start, start + args.batch_size))
...
def topk_eval(args, model, train_data, test_data, user_triple_set, item_triple_set):
    ...
    for user in user_list:
        ...
        while start + args.batch_size <= len(test_item_list):
            ...
            scores ,user_dae_loss,item_dae_loss= model(*_get_feed_data(args, input_data, user_triple_set, item_triple_set, 0, args.batch_size))
